...
import type { AsyncAgentTickStore as AgentTickStore, ClerkIdentityProfile } from '@agent-tick/db';
...
async function processClerkEvent(store: AgentTickStore, config: ServerConfig, type: string, data: Record<string, unknown>): Promise<void> {
  ...
  if (type === 'user.created' || type === 'user.updated') {
    const profile = clerkProfileFromUser(data, issuer);
    if (profile) await store.upsertClerkUser(profile);
    return;
  }
  if (type === 'user.deleted') {
    ...
    if (!userId) return;
    let personalMembership = null as Awaited<ReturnType<AgentTickStore['defaultMembershipForUser']>> | null;
    try {
      personalMembership = await store.defaultMembershipForUser(userId);
    } catch {
      personalMembership = null;
    }
    if (personalMembership?.workspaceType === 'personal') {
      await store.deleteHostedAccountData(userId, personalMembership.workspaceId);
    } else {
      await store.revokeUserAccess(userId);
    }
    return;
  }
...
async function upsertMembership(store: AgentTickStore, config: ServerConfig, data: Record<string, unknown>): Promise<void> {
  ...
  if (!userId) {
    ...
    const profile = clerkProfileFromUser({ ...publicUser, id: subject }, clerkIssuer(config));
    if (!profile) return;
    userId = await store.upsertClerkUser(profile);
  }
...
function clerkProfileFromUser(data: Record<string, unknown>, issuer: string): ClerkIdentityProfile | null {
