...
const eventFallbackPollMs = 15000;
    ...
    await new Promise<void>((resolve) => {
      ...
      void (async () => {
        while (!closed && !abortController.signal.aborted) {
          await eventBus.waitForWorkspaceEvent(eventAuth.workspaceId, eventFallbackPollMs, abortController.signal);
          if (!closed && !abortController.signal.aborted) await sendAuditEvents();
        }
      })().catch(cleanup);
    });
    return reply;
