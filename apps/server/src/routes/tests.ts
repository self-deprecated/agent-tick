...
export async function registerTestActivityRoutes(app: FastifyInstance, { config, store, notifier }: TestActivityRoutesOptions): Promise<void> {
  app.post('/v1/tests', async (request, reply) => {
    ...
    if ((input.context === 'routing_rule' || input.routingRuleId) && membership.role !== 'owner' && membership.role !== 'admin') {
      return reply.status(403).send({ error: { code: 'forbidden', message: 'Workspace Owner or Admin role required', requestId: request.id } });
    }
    ...
    const sessionId = testSessionId(input.context ?? 'setup', route);
    const session = { title: workflowLabel };
    ...
    const requestRecord = await store.createRequest({
      ...
      metadata: { agentTickTest: 'true', context: input.context ?? 'setup', firstParty: 'true' },
      isTest: true,
      testLabel: workflowLabel,
      ...(auth.userId ? { userId: auth.userId } : {})
    });
...
function testSessionId(context: string, route: { agentTokenId?: string; routingRuleId?: string }): string {
  return ['agent_tick_test', context, route.routingRuleId ?? route.agentTokenId ?? 'workspace'].join(':');
}
