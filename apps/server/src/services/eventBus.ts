...
export function publishAuditWrites<T extends { writeAuditEvent: (...args: any[]) => unknown }>(store: T, eventBus: WorkspaceEventBus): void {
  ...
  const afterCommit = (store as T & { afterCommit?: (run: () => void) => void }).afterCommit?.bind(store as T & { afterCommit?: (run: () => void) => void });
  store.writeAuditEvent = ((workspaceId: string, ...rest: unknown[]) => {
    const publish = () => patchable[publisherMarker]?.(workspaceId);
    const publishAfterDurableWrite = () => afterCommit ? afterCommit(publish) : publish();
    const result = original(workspaceId, ...rest);
    if (isPromiseLike(result)) return Promise.resolve(result).then((value) => { publishAfterDurableWrite(); return value; });
