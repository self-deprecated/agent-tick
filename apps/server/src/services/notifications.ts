...
export function createExpoPushNotifier({ store, fetch: fetchImpl = globalThis.fetch, endpoint = 'https://exp.host/--/api/v2/push/send', logger }: ExpoPushNotifierOptions): RequestNotifier {
  return {
    async notifyRequestCreated(request) {
      if (!fetchImpl) return;
      const devices = request.deliveryKind === 'audience_channel' && request.audienceChannelId
        ? await store.listPushDevicesForAudienceChannel(request.audienceChannelId)
        : await store.listPushDevicesForRequestRecipients(request.id);
      ...
      const response = await fetchImpl(endpoint, {
        ...
        body: JSON.stringify(targets.map((to) => ({
          ...
          data: { requestId: request.id, workspaceId: request.workspaceId, type: 'request' }
        })))
      });
    ...
    async notifyRequestCreated(request) {
      ...
      const response = await fetchImpl(url, {
        ...
        body: JSON.stringify({
          ...
          request: { id: request.id, title: request.title, body: request.body, command: request.command, requester: request.requester, risk: request.risk, metadata: request.metadata, createdAt: request.createdAt },
