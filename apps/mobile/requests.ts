...
export function mobileRequestKey(request: { id: string; connectionID?: string; workspaceId?: string }): string {
  return mobileRequestSelectionKey(request.id, request.connectionID, request.workspaceId);
}

export function mobileRequestSelectionKey(requestID: string, connectionID?: string | null, workspaceID?: string | null): string {
  if (connectionID) {
    const workspacePart = workspaceID ? `${encodeURIComponent(workspaceID)}:` : "";
    return `${encodeURIComponent(connectionID)}:${workspacePart}${encodeURIComponent(requestID)}`;
  }
  if (workspaceID) return `workspace:${encodeURIComponent(workspaceID)}:${encodeURIComponent(requestID)}`;
  return requestID;
}

export function mobileRequestMatchesSelection(request: MobileRequest, selectionID: string | null): boolean {
  if (!selectionID) return false;
  return mobileRequestKey(request) === selectionID ||
    mobileRequestSelectionKey(request.id, request.connectionID) === selectionID ||
    mobileRequestSelectionKey(request.id, undefined, request.workspaceId) === selectionID ||
...
export function normalizeRequest(value: unknown): MobileRequest {
  ...
  return {
    ...
    deliveryKind: request.deliveryKind === "audience_channel" ? "audience_channel" : "routed_members",
    responsePolicy: request.responsePolicy === "deadline_plurality" || request.responsePolicy === "first_response" ? request.responsePolicy : "quorum",
...
export function requestAgentLabel(request: MobileRequest) {
  return request.metadata?.agentTokenLabel || request.requester.name || "Agent";
}

export function requestRoutingLabel(request: MobileRequest) {
  return request.testLabel || metadataLabel(request, ["routingRuleName"]) || (request.metadata?.routingRuleId ? "Routing Rule" : "");
}

export function requestOwnerLabel(request: MobileRequest) {
  return metadataLabel(request, ["ownerName"]) || (request.metadata?.ownerUserId || request.metadata?.userId ? "Workspace Owner" : "");
}
...
export function shouldSuppressResponseProgress(request: MobileRequest) {
  if (request.status !== "pending") return false;
  if (request.workspaceMemberCount === 1) return true;
  const quorum = request.quorum;
  return Boolean(
    quorum &&
    quorum.requiredResponseCount === 1 &&
    quorum.waitingFor <= 1 &&
    quorum.currentUserResponded !== true &&
    quorum.currentUserEligible !== false,
  );
}

export function quorumProgressMessage(request: MobileRequest) {
  const quorum = request.quorum;
  if (!quorum || shouldSuppressResponseProgress(request)) return "";
...
function responseHistoryLabel(response: ResponseRecord) {
  ...
  return `Member response ${responsePastTense(response.choiceId)}${source}`;
}
