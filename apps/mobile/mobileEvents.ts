...
export function subscribeToMobileEventStream({
  ...
  const poll = async () => {
    ...
    try {
      const response = await client.pollEvents({ ...(lastEventId !== undefined ? { lastEventId } : {}), timeoutMs, signal: controller.signal });
      if (closed) return;
      activePoll = null;
      hasOpened = true;
      onStatusChange?.("open");
      backoffMs = 1000;
      lastEventId = Math.max(lastEventId ?? 0, response.nextEventId);
