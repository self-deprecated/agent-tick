...
```text
~/.pi/agent/extensions/agent-tick-approval.ts
```
...
The current risky-command patterns include recursive remove commands, `sudo`, broad `chmod`/`chown`, `git`/`jj push`, `docker compose up`, and package-manager install/add commands. Agent Tick commands themselves are ignored so the extension does not approval-loop on its own sanction request.
...
After setup and reload, ask Pi to do normal work that may need approval. For a safe extension smoke test, ask Pi to run a dry-run dependency install only if Agent Tick approves it, for example:

```text
Use Agent Tick if needed, then run npm install --dry-run. Stop if approval is denied or times out.
```

You should receive the sanction in Agent Tick, respond from the mobile app or web UI, and see Pi continue only after approval.
