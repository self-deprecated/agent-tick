...
| Hook fails closed | Agent Tick CLI config/token is missing or the server is unreachable. | Run `agent-tick install`, `agent-tick login`, or `agent-tick config --server ... --token ...`. |
