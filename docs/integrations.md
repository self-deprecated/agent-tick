...
See [Workflow and no-code connector strategy](./workflow-no-code-connector-strategy.md) for the priority order across n8n, Zapier, Make, workflow SDKs, and vertical templates. See [n8n community node plan](./n8n-community-node-plan.md) for the concrete first connector package/operation plan, and [Approval templates](./approval-templates.md) for reusable deploy, refund, outbound email, database migration, and release request shapes.
