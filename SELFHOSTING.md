...
Back up the `agent_tick_data` Docker volume. It contains the SQLite database with users, devices, agent tokens, approval history, policy definitions, approval votes, sessions, and audit events. For a simple backup, stop the container or take a filesystem-consistent snapshot before copying the volume contents.
...
- User mode scopes approvals, devices, pairing tokens, agent tokens, and audit events by user. Organization, team, project, and approval-policy records provide grouping and quorum/sequence approvals for hosted/team setups without changing the single-user flow.
